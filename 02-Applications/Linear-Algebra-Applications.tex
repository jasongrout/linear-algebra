...
Notice that since the first two columns are always pivot columns (since the two vectors form a linearly independent set), the system will always have a single solution. We call the scalars in the linear combination ($\frac{x}{2}$ and $y-\frac{x}{2}$) the \define{coordinates} of the vector with respect to the basis $[(2,1),\, (1,2)]$. For example, the coordinates of $(3,5)$ with respect to the basis $[(2,1),\, (1,2)]$ are $\frac{3}{2}$ and $5-\frac{3}{2}$.  A basis gives us a way of writing any vector by writing the coordinates.
...
Every other vector in $\RR^n$ is a linear combination of these standard basis vectors.  If not specified otherwise, we assume that any coordinates are with respect to this standard basis.  We will find that using a different basis for our coordinates can simplify various problems.  For example, we'll often use eigenvectors as some of our basis vectors.
...
$$.}%
The list of pivot columns of $A^T$, $[(1,2,3),\, (2,0,-2)]$, is a basis for the column space of $A^T$ and the row space of $A$.
...
\begin{example}
Recall example \ref{regression1ex}, in which we found the line closest to passing through the three points $(0,1)$, $(2,4)$, and $(4,5)$.  The first question is, ``Is there a line that passes through all three points?''  If there is a line $y=a_0+a_1x$ that passes through all of the points, then we have a system of equations
\begin{equation*}
...
To be closest to $\vec b$, a vector $\vec r$ with base at $\vec w$ and head at $\vec b$ must meet the plane at a 90 degree angle.  So the vector $\vec r = \vec b-\vec w$ is orthogonal to every column of $A$, hence is in the null space of $A^T$ (so $A^T\vec r=\vec 0$). Solving for $\vec w$ gives us $\vec w = \vec b-\vec r$.  Multiplying both sides on the left by $A^T$ and remembering $A\vec x = \vec w$ gives us the regression equation
$$A^TA\vec x=A^T(\vec b-\vec r) 
= A^T\vec b - A^T\vec r 
= A^T\vec b - \vec 0 =
A^T\vec b.$$
The vector $\vec x$ is precisely the coordinates of $\vec w$ relative to the columns of $A$. Regression is really a problem about projecting vectors onto vector spaces. 
...
$$
A = \begin{bmatrix}1&0\\1&2\\1&4\end{bmatrix},\quad
\vec b = \begin{bmatrix}1\\4\\5\end{bmatrix}, \quad
A^T= \begin{bmatrix}1&1&1\\0&2&4\end{bmatrix}, \quad
