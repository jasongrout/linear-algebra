...
\begin{enumerate}
	\item 
	\marginpar{When working with an abstract inner product, we normally use double bars $\lVert\vec u\rVert$ instead of single bars $|\vec u|$ to talk about the norm of a vector.}
...
\begin{definition}
A matrix is said to be \define{orthogonal} if $A^TA=AA^T=I$, or in other words $A^T=A^{-1}$ (the transpose is the inverse).  
\end{definition}
